...
const unsigned char xormask = 0x55; //3847611839 in hex.
...
static void xor (unsigned char *mem, unsigned int size) {
    unsigned int i;
    for (i = 0; i < size; i++) {
	*((unsigned char *)mem+i) ^= xormask;
    }
}
...
*/
    ...
    while(1) {
		...
		xor(rxringbufptr[rxringbufused], rxringpayload[rxringbufused]);
